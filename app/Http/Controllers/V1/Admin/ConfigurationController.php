    ...
    public function scanProcess2(Request $request, $id = null)
{
    ...
    */
    ...
    try {
        ...
        /*
        |--------------------------------------------------------------------------
        | STEP 1 - PROCESS PDF WITH RULE ENGINE
        |--------------------------------------------------------------------------
        */

        $data = $this->processRuleEngine(
            $config,
            $file,
            $originalName,
            $payload,
        ...
        |
        | This will stop the function immediately after the rule engine
        | has completed.
        |
        | It will NOT download the TXT file.
        | It will NOT update the database with the output file.
        | It will NOT redirect.
        |
        */

        dd([
            'TEST' => 'RULE ENGINE PROCESSING COMPLETED',
            'configuration_id' => $config->id,
            'mapped_txt_file' => $data['Mapped_txt_file'] ?? null,
            'response_keys' => array_keys($data),
            'validation_warnings' => $data['Validation_Warnings'] ?? null,
            'submitted_config' => $data['Submitted_config_json'] ?? null,
        ]);

        /*
        |--------------------------------------------------------------------------
        | STEP 2 - DOWNLOAD GENERATED TXT
        |--------------------------------------------------------------------------
        |
        | REMOVE THE dd() ABOVE AFTER YOUR TEST.
        |
        */
        ...
        $txtContent = $this->downloadGeneratedTxt(
            $config,
            $filename,
            $baseUrl
        );
        ...
        */
        ...
        $config->update([
            'configured_data'   => $data['Submitted_config_json'] ?? null,
            'validation_data'   => $data['Validation_Warnings'] ?? null,
        ...
        */
        ...
        Log::info('REDIRECTING TO FINAL PROCESS', [
            'configuration_id' => $config->id,
...
/**
 * Process the PDF using the rule engine.
 */
private function processRuleEngine(
    $config,
    $file,
    $originalName,
    $payload,
    ...
    Log::info('RULE ENGINE PROCESSING STARTED', [
        'configuration_id' => $config->id,
        'file' => $file,
    ...
    $configJson = json_encode($payload);
    ...
    Log::info('RULE ENGINE PROCESSING COMPLETED', [
        'configuration_id' => $config->id,
    ...
    if (!$response->successful()) {
        ...
        Log::error('RULE ENGINE PROCESSING FAILED', [
            'configuration_id' => $config->id,
            'status' => $response->status(),
...
/**
 * Download the generated TXT file from the rule engine.
 */
private function downloadGeneratedTxt(
    $config,
    $filename,
    $baseUrl
) {
